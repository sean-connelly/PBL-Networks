...
thumbnailImage: http://res.cloudinary.com/dfngkha36/image/upload/c_scale,w_750/v1527957250/StockSnap_PBLSunshine.jpg
...
tmap::tmap_options(tmap.limits = c(facets.plot = 64,facets.view = 10))
...
#Bike networks
dc <- st_read("Shapefiles/Washington.shp",quiet = T)
seattle <- st_read("Shapefiles/Seattle.shp",quiet = T)
nyc <- st_read("Shapefiles/New_York_City.shp",quiet = T)
madison <- st_read("Shapefiles/Madison.shp",quiet = T)
sfca <- st_read("Shapefiles/San_Francisco.shp",quiet = T)
pdx <- st_read("Shapefiles/Portland.shp",quiet = T)
...
<font color="white">.<p>.<p>.<p>.<p>.<p>.<p>.<p>.<p>.<p>.<p>.<p>.<p>.<p>..<p>.<p>.<p>.<p>.
